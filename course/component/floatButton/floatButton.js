...
Component({
  ...
  methods: {
    ...
    getData() {
      let this_ = this;
      util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${util.posId}&r=${postr}.my.index&openid=${wx.getStorageSync('openid')}`, 'POST', {
      }).then((res) => {
      ...
      wx.scanCode({
        success(res) {
          ...
          if (arr[0] == "1" || arr[0] == "3" || arr[0] == "6" ) {
            ...
            util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${util.posId}&r=${postr}.hx.index&openid=${wx.getStorageSync('openid')}`, 'POST', {
            ...
            util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${util.posId}&r=${postr}.index.hexiao&openid=${wx.getStorageSync('openid')}`, 'POST', {
              str: parameter[0].scene
            }).then((res) => {
              console.log('韩磊', res)
