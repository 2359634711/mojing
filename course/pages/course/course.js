...
Page({
  ...
  playVideo(e) {
    ...
    util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${util.posId}&r=${postr}.ykt.gksp&openid=${wx.getStorageSync('openid')}`, 'POST', {
      ykt_id: id
    }).then((res) => {
      console.log(res.res)
    }).catch(err => {
  ...
  getVideoList() {
    ...
    util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${util.posId}&r=mojing.ykt.xuexi&openid=${wx.getStorageSync('openid')}`, 'POST', {
      page: page,
      type: 0
    }).then((res) => {
      console.log(res)
      ...
      this_.setData({
        play: res.top ? res.top : res.list[0],
        videoList: newList,
        videoPage: page
      })
  ...
  save(e) {
    ...
    util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${util.posId}&r=${postr}.ykt.dzjl&openid=${wx.getStorageSync('openid')}`, 'POST', {
      ykt_id: newData.id
    }).then((res) => {
      console.log(res)
      newData.status = res.status;
      res.status == '0' ? newData.num++ : newData.num--;
