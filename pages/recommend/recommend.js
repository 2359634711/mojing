...
Page({
  ...
  getData: function(idx) {
    ...
    util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${api.postI}&r=mojing.tuijian.index&openid=${app.globalData.openid}`, 'POST', {
      type: idx
    }).then((res) => {
      this.setData({
        shenfen: res.shenfen
  ...
  storage(e) {
    ...
    let imgList = []
    util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${api.postI}&r=mojing.tuijian.save&openid=${app.globalData.openid}`, 'POST', {
  ...
  onShareAppMessage: function(e) {
    ...
    if (e.target) {
      ...
      util.request(util.apiUrl + `app/ewei_shopv2_api.php?i=${api.postI}&r=mojing.tuijian.fenxiang&openid=${app.globalData.openid}`, 'POST', {
        id: id,
        type: 1
      }).then((res) => {
        console.log(res)
